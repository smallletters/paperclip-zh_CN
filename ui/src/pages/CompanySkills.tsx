    ...
    <div className="border-b border-border px-4 py-4">
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          >
            {isPending ? t("status.creating") : t("btn.createSkill")}
          </Button>
        </div>
      </div>
    </div>
...
function SkillPane({
  ...
  if (!detail) {
    ...
    return (
      <EmptyState
        icon={Boxes}
        message={t("page.skills.selectSkill")}
      />
    );
  }
    ...
    <div className="min-w-0">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {detail.editable ? (
            ...
            >
              ...
              {editMode ? t("btn.stopEditing") : t("btn.edit")}
            </button>
          ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("label.source")}</span>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("label.pin")}</span>
                ...
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  ...
                  >
                    ...
                    {t("btn.installUpdate")}{latestPin ? ` ${latestPin}` : ""}
                  </Button>
                )}
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("btn.upToDate")}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("label.key")}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("label.mode")}</span>
              <span>{detail.editable ? t("label.editable") : t("label.readOnly")}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("label.usedBy")}</span>
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("label.noAgentsAttached")}</span>
            ) : (
            ...
            {editMode && file?.editable && (
              <>
                ...
                <Button size="sm" onClick={onSave} disabled={savePending}>
                  ...
                  {savePending ? t("status.saving") : t("btn.save")}
                </Button>
              </>
            )}
          </div>
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">{t("page.skills.selectFile")}</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
...
export function CompanySkills() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("page.skills.title"), href: "/skills" },
      ...(routeSkillId ? [{ label: t("page.skills.detail") }] : []),
    ]);
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("msg.skillsImported"),
        body: `${result.imported.length} ${result.imported.length === 1 ? t("label.skill") : t("label.skills")} ${t("msg.added")}.`,
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("msg.importWarnings"), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("msg.skillImportFailed"),
        body: error instanceof Error ? error.message : t("msg.importSkillFailed"),
      });
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("msg.skillCreated"),
        body: `${skill.name} ${t("msg.nowEditable")}.`,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("msg.skillCreationFailed"),
        body: error instanceof Error ? error.message : t("msg.createSkillFailed"),
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: () => {
      setScanStatusMessage(t("msg.scanningWorkspaces"));
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("msg.refreshingList"));
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
      const summary = formatProjectScanSummary(result);
      setScanStatusMessage(summary);
      pushToast({
        tone: "success",
        title: t("msg.scanComplete"),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("msg.skillConflicts"),
          body: result.conflicts[0].reason,
        });
      } else if (result.warnings[0]) {
        pushToast({
          tone: "warn",
          title: t("msg.scanWarnings"),
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: t("msg.scanFailed"),
        body: error instanceof Error ? error.message : t("msg.scanProjectFailed"),
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("msg.saveFailed"),
        body: error instanceof Error ? error.message : t("msg.saveSkillFailed"),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("msg.skillUpdated"),
        body: skill.sourceRef ? `${t("label.pinnedTo")} ${shortRef(skill.sourceRef)}` : skill.name,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("msg.updateFailed"),
        body: error instanceof Error ? error.message : t("msg.installUpdateFailed"),
      });
    },
  });

  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("page.skills.selectCompany")} />;
  }
    ...
    <>
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("page.skills.addSkillSource")}</DialogTitle>
            <DialogDescription>
              {t("page.skills.addSkillSourceDesc")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            ...
            >
              <span>
                <span className="block font-medium">{t("page.skills.browseSkillsSh")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("page.skills.browseSkillsShDesc")}
                </span>
              </span>
            ...
            >
              <span>
                <span className="block font-medium">{t("page.skills.searchGithub")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("page.skills.searchGithubDesc")}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
        ...
        <aside className="border-r border-border">
          <div className="border-b border-border px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                <h1 className="text-base font-semibold">{t("page.skills.title")}</h1>
                <p className="text-xs text-muted-foreground">
                  {t("page.skills.available", { count: String(skillsQuery.data?.length ?? 0) })}
                </p>
              </div>
              <div className="flex items-center gap-1">
                <Button
                  ...
                  title={t("page.skills.scanWorkspaces")}
                >
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              ...
              <input
                ...
                placeholder={t("page.skills.filter")}
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              <input
                ...
                placeholder={t("page.skills.addSource")}
                className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              />
              <Button
                size="sm"
              ...
              >
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("btn.add")}
              </Button>
            </div>
            {scanStatusMessage && (
