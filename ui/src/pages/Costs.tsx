  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("label.financeLedger")}</CardTitle>
        <CardDescription>
          {t("label.financeLedgerDesc")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("label.debits")}
          value={formatCents(debitCents)}
          subtitle={t("label.debitsSubtitle", { count: String(eventCount), plural: eventCount === 1 ? "" : "s" })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("label.credits")}
          value={formatCents(creditCents)}
          subtitle={t("label.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("label.net")}
          value={formatCents(netCents)}
          subtitle={t("label.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("label.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("label.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("label.to")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("label.inferenceSpend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("label.tokensAcrossEvents", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("label.agentsPaused", { agents: String(budgetData?.pausedAgentCount ?? 0), projects: String(budgetData?.pausedProjectCount ?? 0) })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("label.budgetUsage", { spent: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
                    : t("label.noMonthlyCap")
              }
              icon={Coins}
            />
            <MetricTile
              label={t("label.financeNet")}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("label.financeNetSubtitle", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("label.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("label.estimatedInRange", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="budgets">{t("tab.budgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("tab.providers")}</TabsTrigger>
          <TabsTrigger value="billers">{t("tab.billers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("tab.finance")}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("page.costs.selectDate")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("label.inferenceLedger")}</CardTitle>
                    <CardDescription>
                      {t("label.inferenceLedgerDesc")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? `${t("label.budget")} ${formatCents(spendData.summary.budgetCents)}`
                            : t("label.unlimitedBudget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("label.usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("label.budgetConsumed", { percent: String(spendData.summary.utilizationPercent) })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("label.byAgent")}</CardTitle>
                      <CardDescription>{t("label.byAgentDesc")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byAgent.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("label.noCostEvents")}</p>
                      ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("label.in")} {formatTokens(row.inputTokens + row.cachedInputTokens)} · {t("label.out")} {formatTokens(row.outputTokens)}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? `${row.apiRunCount} ${t("label.api")}` : `0 ${t("label.api")}`}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? `${row.subscriptionRunCount} ${t("label.subscription")}`
                                      : `0 ${t("label.subscription")}`}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens)} {t("label.tok")}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("label.byProject")}</CardTitle>
                      <CardDescription>{t("label.byProjectDesc")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("label.noProjectCosts")}</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("label.noFinanceEvents")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("label.budgetControlPlane")}</CardTitle>
                  <CardDescription>
                    {t("label.budgetControlPlaneDesc")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    label={t("label.activeIncidents")}
                    value={String(activeBudgetIncidents.length)}
                    subtitle={t("label.activeIncidentsSubtitle")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("label.pendingApprovals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("label.pendingApprovalsSubtitle")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("label.pausedAgents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("label.pausedAgentsSubtitle")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("label.pausedProjects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("label.pausedProjectsSubtitle")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("label.activeIncidentsTitle")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("label.activeIncidentsDesc")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`label.${scopeType}Budgets`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {t(`label.${scopeType}BudgetDesc`)}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("label.noCostEventsPeriod")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("label.noBillableEvents")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("label.byBiller")}</CardTitle>
                      <CardDescription>{t("label.byBillerDesc")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("label.noFinanceEventsSimple")}</p>
                      ) : (
