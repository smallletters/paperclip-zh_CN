...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("page.instanceSettings.heartbeats") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("page.instanceSettings.failedUpdateHeartbeat"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("page.instanceSettings.failedDisableAll"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("page.instanceSettings.loadingHeartbeats")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("page.instanceSettings.failedLoadHeartbeats")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("page.instanceSettings.agentsWithTimer")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("page.instanceSettings.active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("page.instanceSettings.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("page.instanceSettings.company") : t("page.instanceSettings.companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("page.instanceSettings.agent") : t("page.instanceSettings.agents");
              if (!window.confirm(t("page.instanceSettings.disableConfirm", { count: String(enabledCount), noun }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("page.instanceSettings.disabling") : t("page.instanceSettings.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("page.instanceSettings.noHeartbeatsMatch")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("page.instanceSettings.on") : t("page.instanceSettings.off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("page.instanceSettings.never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("page.instanceSettings.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("page.instanceSettings.disableTimerHeartbeat") : t("page.instanceSettings.enableTimerHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
